...
import { ChevronRight, ChevronLeft, Maximize2, Lock, ArrowRight, Mail, Check } from 'lucide-react';
    ...
    <div className="w-screen h-screen bg-slate-50 text-slate-800 flex items-center justify-center relative overflow-hidden font-body selection:bg-blue-100">
      ...
      {current > 0 && (
        <motion.div
          ...
          className="absolute top-8 left-10 flex items-center gap-3 z-20"
        >
          <TreenityLogo className="w-10 h-auto text-slate-900" />
          <span className="font-tech text-sm font-bold tracking-[0.2em] text-slate-900 uppercase">Treenity</span>
        </motion.div>
      )}
          ...
          >
            ...
            {slideData.layout === 'cover' && (
              <div className="flex flex-col items-center justify-center text-center h-full">
                <motion.div variants={itemVariants} className="flex flex-col md:flex-row items-center gap-6 mb-12">
                    <div className="w-60 h-60 md:w-80 md:h-80">
                        <TreenityLogo className="w-full h-full text-slate-900" />
                    </div>
                    <h1 className="font-tech text-7xl md:text-[9rem] text-slate-900 tracking-tight leading-none pt-4">
                      Treenity
                    </h1>
                </motion.div>
            ...
            {slideData.layout === 'grid' && (
              <div className="w-full max-w-6xl mx-auto">
                <motion.div variants={titleVariants} className="text-center mb-16">
                  <h2 className="font-tech text-5xl text-slate-900 mb-3">{slideData.title}</h2>
                ...
                <div className="md:col-span-5">
                  <motion.div variants={titleVariants}>
                    ...
                    <h2 className="font-tech text-6xl md:text-7xl text-slate-900 leading-[1.1] font-bold uppercase mb-6">
                      ...
                      <span className={slideData.type === 'problem' ? 'text-red-600' : 'text-green-600'}>
                      ...
                      <motion.div key={idx} variants={itemVariants} className="flex items-center gap-6 group">
                        ...
                        <div className={`w-[60px] h-[60px] flex-shrink-0 flex items-center justify-center rounded-full text-white shadow-lg z-10 relative transition-transform duration-300 group-hover:scale-110
                          ...
                          {step.icon ? (
                            ...
                            slideData.type === 'problem' ? step.id : <Check size={24} strokeWidth={2.5} />
                          )}
            ...
            {slideData.layout === 'features_grid' && (
              <div className="max-w-6xl mx-auto w-full">
                <motion.div variants={titleVariants} className="text-center mb-12">
                  <h2 className="font-tech text-5xl text-slate-900 mb-4">{slideData.title}</h2>
                  <p className="text-slate-500 text-xl">{slideData.subtitle}</p>
                </motion.div>
            ...
            {slideData.layout === 'timeline_new' && (
              <div className="max-w-[90rem] mx-auto w-full">
                <motion.h2 variants={titleVariants} className="font-tech text-6xl text-center mb-20 text-slate-900">{slideData.title}</motion.h2>
            ...
            {slideData.layout === 'price_hero' && (
              <div className="flex items-center justify-center w-full h-full">
                <motion.div
                  ...
                  className="bg-white px-24 py-16 rounded-[2.5rem] shadow-2xl shadow-blue-100 border border-slate-100 relative overflow-hidden text-center max-w-4xl w-full"
                >
                  ...
                  <h3 className="text-slate-400 font-bold uppercase tracking-[0.3em] mb-4 text-xs mt-4">Investimento</h3>
                  ...
                  <p className="text-slate-400 text-sm mb-10">{slideData.obs}</p>

                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="bg-blue-600 text-white px-12 py-4 rounded-full font-bold text-lg shadow-lg shadow-blue-200 transition-all flex items-center gap-2 mx-auto"
                  >
            ...
            {slideData.layout === 'thanks_hero' && (
              <div className="flex flex-col items-center justify-center text-center h-full">
                <motion.h2 variants={titleVariants} className="font-tech text-7xl md:text-8xl text-slate-900 mb-4 tracking-wide uppercase">
                  {slideData.title}
                </motion.h2>
